  ...
  return (
    <header className="max-w-4xl mx-auto mb-6">
      <div className="w-full rounded overflow-hidden bg-gray-100 h-56 flex items-center justify-center">
        {img ? (
          <img src={img} alt={station?.StationName || "Station"} className="w-full h-full object-cover" />
        ) : (
      ...
      <div className="text-sm text-gray-600">{station?.State} {station?.State && station?.District ? "• " : ""} {station?.District}</div>
    </header>
  );
}
