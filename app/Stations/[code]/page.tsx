// app/Stations/[code]/page.tsx (server component)
...
export default async function StationPage({ params }: Props) {
  ...
  const BASE = process.env.NEXT_PUBLIC_APP_URL || ""; // ensure this exists in env for server fetch
  ...
  return (
    <main className="p-6">
      <section className="max-w-4xl mx-auto">
        <div className="mb-6">
          {station.image_url ? (
            <img src={station.image_url} alt={`${station.name} photo`} className="w-full h-48 object-cover rounded" />
          ) : (
            <div className="w-full h-48 bg-gray-100 rounded flex items-center justify-center">No image</div>
          )}
          <div className="mt-4">
            <h1 className="text-2xl font-bold">{station.code} — {station.name}</h1>
