...
export default function RestroMenuClient({ header, items, offer }: Props) {
  ...
  // ✅ outlet meta ko sessionStorage me save karo (checkout/review ke liye)
  useEffect(() => {
    if (typeof window === "undefined") return;
    const outletMeta = {
      restroCode: header.restroCode,
      restroName: header.outletName,
  ...
  }, [header.restroCode, header.outletName, header.stationCode, header.stationName]);
  ...
  return (
    <>
      ...
      <style jsx>{`
        ...
        @media (min-width: 1024px) { .mobile-pill { display:none; } }
        @media (max-width: 640px) { .title { font-size:1.4rem; } .subtitle { font-size:0.9rem; } }
      `}</style>
