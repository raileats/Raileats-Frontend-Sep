...
export async function GET(req: Request, { params }: { params: { code: string } }) {
  ...
  const { data: station, error: stErr } = await supabase
    .from("stations")
    .select("code, name, state, district, storage_image_path")
    .eq("code", code)
    .maybeSingle();
  ...
  for (const r of restros || []) {
    ...
    if (rfErr) {
      // skip this restro on error, but continue others
  ...
  // 4) build public URLs for images (adjust bucket names)
  const makeUrl = (path?: string | null) => {
    if (!path) return null;
    try {
      // If images are in 'public' bucket or made public
