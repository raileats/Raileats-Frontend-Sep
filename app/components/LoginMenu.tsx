        ...
        <div className="relative">
          ...
          {open && (
            <div className="absolute right-0 mt-2 w-56 rounded-md border bg-white shadow-lg">
              <a href="/profile" className="block px-3 py-2 hover:bg-gray-50">My Profile</a>
              <a href="/orders" className="block px-3 py-2 hover:bg-gray-50">My Orders</a>
              <a href="/wallet" className="block px-3 py-2 hover:bg-gray-50">My Wallet</a>
        ...
        {step === "phone" ? (
          <>
            <input value={phone} onChange={(e)=>setPhone(e.target.value)}
              className="w-full rounded-md border px-3 py-2" placeholder="Enter phone number"/>
            {error && <p className="mt-2 text-sm text-red-600">{error}</p>}
            <button onClick={sendOtp} className="mt-3 w-full rounded-md bg-yellow-600 py-2 text-white">Send OTP</button>
            <p className="mt-2 text-xs text-gray-500">Demo OTP: <b>111111</b></p>
          </>
        ) : (
          <>
            <input value={otp} maxLength={6} onChange={(e)=>setOtp(e.target.value)}
              className="w-full rounded-md border px-3 py-2 tracking-widest text-center" placeholder="Enter 6-digit OTP"/>
            {error && <p className="mt-2 text-sm text-red-600">{error}</p>}
            <button onClick={verifyOtp} className="mt-3 w-full rounded-md bg-yellow-600 py-2 text-white">Verify & Login</button>
          </>
        )}
      </div>
    </div>
