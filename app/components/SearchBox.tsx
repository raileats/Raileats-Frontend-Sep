...
export default function SearchBox() {
  ...
  async function fetchStations(trainNo: string) {
    try {
      ...
      const list = (j.rows || [])
        ...
        .filter((s: any) => s.restro > 0); // ✅ only valid
  ...
  function handleTrainSelect(t: any) {
    setSelectedTrain(t);
    setInputValue(`${t.trainNumber} - ${t.trainName}`);
    fetchStations(t.trainNumber);
  }
  ...
  const handleSearch = () => {
    ...
    if (searchType === "train") {
      ...
      const slug = makeTrainSlug(selectedTrain.trainNumber);
